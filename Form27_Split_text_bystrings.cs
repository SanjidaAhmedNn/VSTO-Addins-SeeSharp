...
namespace VSTO_Addins
{
    ...
    public partial class Form27_Split_text_bystrings
    {
        ...
        public Form27_Split_text_bystrings()
        {
            InitializeComponent();
        }
        ...
        private object SplitText(object Source, object Separators, object Consecutive, object KeepSeparator, object Before)
        {
            ...
            {
                if (Conversions.ToBoolean(MatchArr(Separators, Source, i)((object)0)))
                {
                    Index = Index + 1;
                    Array.Resize(ref SplitValues, Index + 1);
                    Separator = Conversions.ToString(MatchArr(Separators, Source, i)((object)1));
                    if (Conversions.ToBoolean(Operators.ConditionalCompareObjectEqual(KeepSeparator, true, false)))
                    {
                        if (Conversions.ToBoolean(Operators.ConditionalCompareObjectEqual(Before, true, false)))
                        {
                            SplitValues[Index] = Strings.Mid(Conversions.ToString(Source), Position, i - Position);
                        }
                        else
                        {
                            SplitValues[Index] = Strings.Mid(Conversions.ToString(Source), Position, i - Position + Strings.Len(Separator));
                        }
                    }
                    else
                    {
                    ...
                    i = i + Strings.Len(Separator) - 1;
                    if (Conversions.ToBoolean(Operators.ConditionalCompareObjectEqual(Consecutive, true, false)))
                    {
                        while (MatchArr(Separators, Source, i + 1)((object)0))
                        {
                            Separator = Conversions.ToString(MatchArr(Separators, Source, i + 1)((object)1));
                            i = i + Strings.Len(Separator);
                        }
                    }
                    if (Conversions.ToBoolean(Operators.ConditionalCompareObjectEqual(KeepSeparator, true, false)))
                    {
                        if (Conversions.ToBoolean(Operators.ConditionalCompareObjectEqual(Before, true, false)))
                        {
                            Position = i + 1 - Strings.Len(Separator);
                        }
                        else
                        {
                            Position = i + 1;
            ...
            if (Strings.Len(Source) + 1 - Position >= 0)
            {
                Index = Index + 1;
                Array.Resize(ref SplitValues, Index + 1);
                SplitValues[Index] = Strings.Mid(Conversions.ToString(Source), Position, Strings.Len(Source) + 1 - Position);
            }
        ...
        private object SplitCount(object Source, object Separators, object Consecutive)
        {
            ...
            for (int i = 1, loopTo = Strings.Len(Source); i <= loopTo; i++)
            {
                if (Conversions.ToBoolean(MatchArr(Separators, Source, i)((object)0)))
                {
                    ...
                    if (Conversions.ToBoolean(Operators.ConditionalCompareObjectEqual(Consecutive, true, false)))
                    {
                        while (MatchArr(Separators, Source, i)((object)0))
                        {
                            Separator = Conversions.ToString(MatchArr(Separators, Source, i)((object)1));
                            i = i + Strings.Len(Separator);
                        }
        ...
        private void Display()
        {
            ...
            if ((X1 | X2) & !string.IsNullOrEmpty(TB_TypeString.Text))
            {

                string[] OldSeparators;
                OldSeparators = Strings.Split(TB_TypeString.Text, Microsoft.VisualBasic.Constants.vbNewLine);

                var Separators = new string[1];
                int Indx = -1;

                for (int i = Information.LBound(OldSeparators), loopTo1 = Information.UBound(OldSeparators); i <= loopTo1; i++)
                {
                    if (!string.IsNullOrEmpty(OldSeparators[i]))
                    {
                        Indx = Indx + 1;
                        Array.Resize(ref Separators, Indx + 1);
                ...
                if (X1)
                {
                    ...
                    for (int i = 1, loopTo2 = r; i <= loopTo2; i++)
                    {
                        ...
                        string[] SplitValues;
                        SplitValues = (string[])SplitText(source, Separators, Consecutive, KeepSeparator, Before);
                        for (int m = Information.LBound(SplitValues), loopTo3 = Information.UBound(SplitValues); m <= loopTo3; m++)
                        {
                            Index = Index + 1;
                    ...
                    double abscissa1 = 0d;
                    double abscissa2 = 0d;
                    for (int i = 1, loopTo4 = r; i <= loopTo4; i++)
                    {
                        string source = Conversions.ToString(displayRng.Cells[i, 1].value);
                        string[] SplitValues;
                        SplitValues = (string[])SplitText(source, Separators, Consecutive, KeepSeparator, Before);
                        ...
                        for (int m = Information.LBound(SplitValues) + 1, loopTo5 = Information.UBound(SplitValues); m <= loopTo5; m++)
                        {
                ...
                else if (X2)
                {
                    ...
                    {
                        string source = Conversions.ToString(displayRng.Cells[i, 1].value);
                        lengths[i - 1] = Conversions.ToInteger(SplitCount(source, Separators, Consecutive));
                    }
                    ...
                    for (int i = 1, loopTo9 = displayRng.Rows.Count; i <= loopTo9; i++)
                    {
                        ...
                        string[] SplitValues;
                        SplitValues = (string[])SplitText(source, Separators, Consecutive, KeepSeparator, Before);
                        for (int j = Information.LBound(SplitValues), loopTo10 = Information.UBound(SplitValues); j <= loopTo10; j++)
                            Values[i - 1, j] = SplitValues[j];
                    }
                    for (int j = 0, loopTo11 = TotalWidth - 1; j <= loopTo11; j++)
                    {
                        ...
                        for (int i = 0, loopTo13 = r - 1; i <= loopTo13; i++)
                        {
                            var label1 = new System.Windows.Forms.Label();
        ...
        private void Btn_OK_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                if ((X1 | X2) & !string.IsNullOrEmpty(TB_TypeString.Text))
                {
                    ...
                    for (int i = Information.LBound(OldSeparators), loopTo = Information.UBound(OldSeparators); i <= loopTo; i++)
                    {
                        if (!string.IsNullOrEmpty(OldSeparators[i]))
                        {
                            Indx = Indx + 1;
                    ...
                    if (X1)
                    {
                        ...
                        for (int i = 1, loopTo3 = r; i <= loopTo3; i++)
                        {
                            ...
                            for (double m = 1d, loopTo4 = Conversions.ToDouble(Lengths[i - 1]) - 1d; m <= loopTo4; m++)
                            {
                                RowNumber = RowNumber + 1;
                        ...
                        rng2.Select();
                        for (int j = 1, loopTo5 = rng2.Columns.Count; j <= loopTo5; j++)
                    ...
                    else if (X2)
                    {
                        ...
                        for (int i = 1, loopTo6 = r; i <= loopTo6; i++)
                        {
                            ...
                            for (int m = Information.LBound(SplitValues), loopTo7 = Information.UBound(SplitValues); m <= loopTo7; m++)
                        ...
                        for (int i = 1, loopTo8 = r; i <= loopTo8; i++)
                        {
                            ...
                            {
                                for (int m = 1, loopTo10 = MaxColumns; m <= loopTo10; m++)
                        ...
                        rng2.Select();
                        for (int j = 1, loopTo11 = rng2.Columns.Count; j <= loopTo11; j++)
                            rng2.Columns[j].Autofit();
        ...
        private void TB_TypeString_TextChanged(object sender, EventArgs e)
        {
        ...
        private void Form27_Split_text_bystrings_Closing(object sender, CancelEventArgs e)
        {
            GlobalModule.form_flag = false;
        }

        private void Form27_Split_text_bystrings_Disposed(object sender, EventArgs e)
        {
            GlobalModule.form_flag = false;
        }

        private void Form27_Split_text_bystrings_Shown(object sender, EventArgs e)
        {
            Focus();
            BringToFront();
            Activate();
